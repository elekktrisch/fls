...
INSERT INTO Clubs
           ...
           ,[DeletedByUserId]
           ,[RecordState]
           ,[OwnerId], [OwnershipType], [ClubStateId], [SendDeliveryMailExportTo], [SendPlanningDayInfoMailTo])
     VALUES
           (@insertClubId
           ,'Test-Club'
		   ,'TestClub'
...
INSERT INTO PersonClub (PersonId, ClubId, MemberNumber, IsMotorPilot, IsTowPilot, IsGliderInstructor, IsGliderPilot, IsGliderTrainee, CreatedOn, CreatedByUserId, RecordState, OwnerId, OwnershipType, IsPassenger, IsWinchOperator, MemberStateId) VALUES (@personId, @insertClubId, '536594', 0, 0, 1, 1, 0, SYSDATETIME(), @insertUserId, @recordState, @ownerId, @OwnershipType, 0, 0, @memberStateId)
SET @personId = NEWID()
INSERT INTO Persons (PersonId, Lastname, Firstname, Midname, CompanyName, AddressLine1, AddressLine2, Zip, City, Region, CountryId, PrivatePhone, MobilePhone, BusinessPhone, FaxNumber, EmailPrivate, EmailBusiness, Birthday, HasMotorPilotLicence, HasTowPilotLicence, HasGliderInstructorLicence, HasGliderPilotLicence, HasGliderTraineeLicence, CreatedOn, CreatedByUserId, RecordState, OwnerId, OwnershipType, IsFastEntryRecord) 
...
INSERT INTO [dbo].[PlanningDays]
           ([PlanningDayId], [ClubId],[Day],[LocationId],[Remarks],[CreatedOn],[CreatedByUserId],[RecordState],[OwnerId],[OwnershipType],[IsDeleted])
VALUES	(@planningDayId, @insertClubId, (GETDATE()+100), @locationId, 'Test3', SYSDATETIME(),		 @insertUserId, @recordState, @ownerId, @OwnershipType, 0)
...
INSERT INTO [dbo].[AircraftReservations]
           ([AircraftReservationId],[Start],[End],[IsAllDayReservation],[AircraftId],[LocationId],[PilotPersonId],[FlightTypeId],[Remarks],[ClubId],[CreatedOn],[CreatedByUserId],[RecordState],[OwnerId],[OwnershipType],[IsDeleted])
VALUES	(NEWID(), (GETDATE()+1), (GETDATE()+1), 1, (SELECT TOP 1 AircraftId FROM [dbo].[Aircrafts]), @locationId, (SELECT TOP 1 Persons.PersonId FROM Persons LEFT OUTER JOIN
                      PersonClub ON Persons.PersonId = PersonClub.PersonId 
					  where HasGliderPilotLicence = 1 and PersonClub.ClubId = @insertClubId), (SELECT TOP 1 FlightTypeId FROM FlightTypes WHERE ClubId = @insertClubId), 'Test',@insertClubId, SYSDATETIME(), @insertUserId, @recordState, @ownerId, @OwnershipType, 0)
