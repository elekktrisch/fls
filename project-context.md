...
2. `_bmad-output/planning-artifacts/architecture/architecture-fls-2026-08-29/ARCHITECTURE-SPINE.md`
   — AD-1 to AD-22, the build gates, the depth trigger. **How** the system is built.
...
6. `docs/modernization/` — **not authoritative.** It narrows a search. Take exact legacy behaviour
   from a `legacy-oracle` read against `flsserver/` and `flsweb/`.
...
**`docs/modernization/legacy-migration-plan.md` is attempt-1 material.** Roman retired it on
2026-08-29. Its Destination, Semantics, Owned by, and Notes columns are all rebuild-1. For the legacy
schema use `docs/modernization/legacy-tables/` — 59 folders, each with `_table.json` and one JSON
file per column — and prove every mapping against `flsserver/`.
...
dependency versions are pinned at repository creation.
...
6. **An OIDC provider proves identity; authorization never leaves the product.** Keycloak is the
   reference implementation, behind one `IdentityProvider` port. The role, the club membership, and
   the tenant boundary stay in AlpenFlight tables. Attempt 1 put roles in the provider and lost three
   tables for it. AD-22.
...
- **`AGENTS.md` does not exist yet.** Run `bmad-project-context` against `alpenflight/` once that
  directory has code.
- **The SaaS identity provider is open.** AD-22 requires a Keycloak-compatible OpenID Connect
  provider and fixes Keycloak for local development and the community edition. Decide the SaaS
  product before the first paying club.
