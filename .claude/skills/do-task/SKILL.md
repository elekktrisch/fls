...
**Format + lint the touched files before you commit** — run the project
formatter/linter over the **full glob** you changed (e.g. `prettier --check
"src/**/*.{ts,html,css,json}" "e2e/**/*.{ts,json}"`, the module's `lint`), not just
the one or two files you eyeballed. A format-only miss fails CI a whole round
later — the most wasteful red there is (J-0c T-09 burned a fanout round on exactly
this). Cheap locally, expensive at the gate.
