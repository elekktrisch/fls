...
7. **Test-method-name drift.** Grep the story body for `_test_` / `_check_` / `_pinning_` / `_seeded_` patterns inside backticks (these are test-method-name citations in design notes / test plan). For each cited test, verify it actually exists in the test files via `grep -r "void <name>(" next/server/src/test/`. If a test was renamed during implementation (e.g. `_tsrange_column` → `_tstzrange_column` when the type primitive changed), update the story-body reference OR annotate the citation inline with `(shipped as <new-name> — see Implementation notes)`. Catches the class of finding where future contributors follow the spec to find a pinning test and land on a name that doesn't exist. **Caught in S-014 review** (C#1 + C#2 — 2 stale `_tsrange_column` refs + 5 type-name refs to `tsrange` in design notes).
...
- **Never embed git commit SHAs in committed docs.** Story files, READMEs, ADRs, CONVENTIONS.md, any text that ships in a commit — no `c1ef2f7`-style refs, no "fixed in <commit>", no "introduced by <commit>". The reference is broken by construction (citing commit can't equal cited commit) and squash-merge erases branch SHAs entirely from `main`. Cite by **commit subject** ("fixed in the self-review-fixes commit"), **file:line**, **PR number** (`#N`), or **story ID** (`S-NNN`) instead. SHAs are fine in ephemera — issue comments, PR descriptions, done-report text — never in text that ships in a commit. The `merge_commit:` frontmatter field is explicitly forbidden in modernize-finalize for the same reason; do not reintroduce equivalent SHAs in body text.
