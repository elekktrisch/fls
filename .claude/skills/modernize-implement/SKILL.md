...
- **Code carries its own meaning through naming + structure first** — DDD ubiquitous-language types and methods, Effective Java sizing (small focused classes, intent-revealing names, value objects over primitive obsession). Reach for a rename / extracted type / extracted method **before** a comment. A comment that exists because the identifier is vague is a naming bug, not documentation. Comments earn their keep only when: a hidden invariant the signature can't show, a workaround for a named bug in a named library (with version cited), a regulatory / sacred-cow constraint, or a one-line ADR pointer. Anything else gets cut. Never reference current task / fix / issue.
- **Spell words out — avoid abbreviations and acronyms by default.** `temporaryTableName` over `tempTableName`; `foreignKey` over `fk`; `configuration` over `cfg`; `manager` over `mgr`. The only abbreviations that earn keep are the industry-canonical ones whose long form would read awkwardly (`id` / `uuid` / `url` / `sql` / `http` / `json` / `dto`). Use title-case for multi-word acronyms (`xmlParser`, not `XMLParser` — Effective Java Item 68). Domain acronyms invented inside this codebase get the full form on first use and ideally always; abbreviation laziness compounds over years.
