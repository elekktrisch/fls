...
5. **Naming carries meaning.** Identifiers describe intent in DDD ubiquitous language, not implementation. `processData` / `helper2` / `tmp` / `doIt` = nudge-or-improvement. Also: a name that lets a reader skip the javadoc beats a name that requires it. Boolean-shaped method names (`boolToEnumTag` → `bitToTriStateTag`), generic helper methods (`coerce`, `convert`), or method names that don't say what the return value represents = improvement. **Abbreviations and acronyms are guilty until proven canonical** — `temporaryTableName` over `tempTableName`, `foreignKey` over `fk`, `configuration` over `cfg`, `manager` over `mgr`; the only abbreviations that earn keep are universal (`id`, `uuid`, `url`, `sql`, `http`, `json`, `dto`). Multi-word acronyms in identifiers get title-case (`xmlParser`, not `XMLParser` — Effective Java Item 68). Naming is a design choice, not a stylistic afterthought.
6. **Function + class size + cohesion.** > 50-line methods, god classes (> ~10 deps / responsibilities) flag. Story estimate is context — L story with a 70-line method may be fine; S story with one is a smell. Effective Java sizing: a class that needs a class-level "what this is" javadoc more than 3 short lines is probably two classes; an interface that needs a long contract block is probably under-segregated.
...
10. **Comments + docs.** First question per comment: could naming or structure have carried this meaning without prose? A comment that exists because the identifier is vague = naming bug, flag as improvement (suggest the rename). *What* (not *why*); references to current PR / issue ("added for #42"); docstrings paraphrasing the function name — all improvements. Missing comment where a hidden invariant lives — also improvement. Comments earn keep only when: hidden invariant the signature can't express, workaround for a named bug in a named library (with version), regulatory / sacred-cow constraint, one-line ADR pointer. Anything else gets cut. Be aggressive here — the prevailing failure mode is too many comments, not too few.
