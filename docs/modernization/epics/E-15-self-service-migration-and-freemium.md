...
- **Out:** the schema-mapping logic itself (lives in S-016, shared library `alpenflight/migration-bundle/`). VAT / invoicing / accounting beyond the third-party provider's defaults (operator can iterate later). Per-tenant custom contracts, enterprise plans, volume discounts (future). Anonymous demo writes that should *persist* (deliberately ephemeral — see C27). Mobile-app distribution of the JAR (CLI only).
