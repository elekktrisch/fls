...
acceptance:
  ...
  - System-logs view (legacy `flsweb/src/system/logs/`) ported and reads from the new `t_mutation_audit_event` table (S-027) via `GET /api/v1/admin/audit-events`, filterable by tenant, actor, action, target entity type, time range.
