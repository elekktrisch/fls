...
- [x] **T-08** — `/deliveries` SPA write actions over `DeliveriesStore`: "create deliveries" button (→ POST
...
- [x] **T-10** — **[REALIDP-FLAKE-QUARANTINE]** rider: stabilize/quarantine the 4 chronic real-idp flakes
  ...
  the timeouts).
- [x] **T-11** — Close the money-seam coverage hole gap-hunter C found: (A) the bug-3 fix site `RuleBasedDeliveryDetails.recordCreditConsumption` `Long::sum` (within-delivery glider+tow SUM) is UNTESTED — the e2e "discriminator" only exercises cross-delivery accumulation; the delete fixture matches the credit to the glider immat only so the tow records zero. FIRST establish (empirically, from the engine credit-matching) whether a glider+tow sharing ONE credit within ONE delivery is reachable; if reachable, add a backend test that triggers the within-delivery SUM and REDs if `Long::sum`→overwrite; if UNREACHABLE, escalate (re-classify bug-3 as non-reachable, simplifying the sign-off). (B) add a re-book-of-booked → 409 IT (delete-of-booked is covered, re-book isn't). `./gradlew check` green.
