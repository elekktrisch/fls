...
- [x] **T-08b** — `strip.mjs` is blind to `<!-- -->` comments inside Angular **inline** `template:`/`styles:` literals: they are string content, so neither the stripper nor `--check` sees them. **73 survive across 23 files in `web-src` alone** after T-08, and `--check alpenflight/web/src` still exits reporting only RENAME markers — a green gate over unswept prose. Falsifies the "only directives and `ext:` markers survive" AC and is the fourth silent hole this journey (after `node_modules.windows`, the `build`/`target` exclusion, and T-05b heredocs). Lex inline templates as HTML, keep the token-stream proof over the surrounding literal. Must land before T-15 wires the gate.
  **Also resolve the `@mocked:` conflict** (T-08 stripped 4 of them as prose): `// @mocked: <seam> — <reason>` is a `/do-ship` §4 mock-governance convention that `gap-hunter` greps to tell a declared mock from an undeclared one — deleting it removes a governance mechanism. But by this skill's own standard an unenforced comment is folklore, so do not merely exempt it: **make it a survivor AND have `--check` enumerate the tags**, so the PR's "Mocked seams" list is generated from the code instead of hand-maintained. Must land before T-11 sweeps the specs, where most `@mocked:` tags live.
- [x] **T-08c** — judge pass over the **40 above-threshold entries T-08b stripped without one** (its scope was a strip commit only). Honest reporting is the AC floor; converting the understanding into names is the point. Closes web-src's `aboveThresholdUnreviewed` to 0
- [x] **T-09** — batch `server-test` (3,795 / 319). Strip landed as `5df56914`; the first judging attempt died to a session limit with **zero shard reports**, so its 11 partial files were discarded rather than kept — unaccountable coverage is worse than redone work. Re-running as **manager-dispatched judges over 10 pre-computed shards** (`.comment-strip/shards-plan/server-test-NN.txt`, 315 files / 2,150 entries / 988 ≥8), no coordinator layer, in waves of 4 — a death now costs one shard
...
- [x] **T-18** — the PR's `extract` job is red on a **pre-existing** break, not ours: S-051 flipped `alpenflight/database/tenant-rules.yaml:34` to `PersonClub: tenant-scoped` on 2026-05-24, but `.github/workflows/extract.yml:11,16` path-filters to `alpenflight/database/extract/**`, so that file has never triggered its own gate — last green main run predates the flip. Broken ~3 months, invisible. Fix the stale assertion AND add `tenant-rules.yaml` to the filter so the gate covers its own input. A journey never merges red
