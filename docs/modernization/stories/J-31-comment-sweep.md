...
- [ ] **T-08c** — judge pass over the **40 above-threshold entries T-08b stripped without one** (its scope was a strip commit only). Honest reporting is the AC floor; converting the understanding into names is the point. Closes web-src's `aboveThresholdUnreviewed` to 0
