...
mock_test: alpenflight/web/e2e/tests/forms/validation-hardening.spec.ts   # T-02: single existing spec; T-04 extends to the J-6b alternation `tests/(forms/validation-hardening|persons/persons-edit-membership)` when its spec lands (the derive requires EVERY branch on disk)
...
- [ ] **T-28** — nav chrome fixes (T-01 findings; ordered HERE — prerequisite for T-04/T-05 un-fixme): (a) `navSectionsFor` renders the role UNION for dual-role principals (today `isSystemAdmin` short-circuits → tenant sections hidden, `nav-sections.ts:44-53`); (b) add the missing `/flight-types` nav entry (URL-only screen — the J-7 hollow-screen class) placed per legacy masterdata nav + role visibility; nav-sections spec updated.
- [ ] **T-04** — persons membership data-loss fix: wire `PUT /persons/{id}/clubs/current` into the persons.store update path (fields already hydrated, request omits them) + mock spec case asserting memberNumber/state/role-toggle round-trip. *(T-02 note: if the spec case lands as a NEW `persons/persons-edit-membership.spec.ts`, extend `mock_test:` to the J-6b alternation `tests/(forms/validation-hardening|persons/persons-edit-membership)` in the same commit — the derive requires every branch on disk; if it lands inside `validation-hardening.spec.ts`, no frontmatter change.)*
