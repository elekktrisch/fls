...
- [ ] **T-29** — (T-03 re-baseline revealed) nightly env parity with ci.yml's proof job: the nightly boots the backend as a HOST bootJar but never got ci.yml's host-mapped-Keycloak overrides (ci.yml:727-735 — JWKS/issuer → localhost:8090 AND `keycloak.admin.base-url`, default `http://keycloak:8080`, unreachable from a host process) → ingest 500 ×3 (aircraft/fan-out/flight migration parity) + likely the clubadmin1 users-400 (KC admin client read). Diff the nightly's backend-boot + stack env against ci.yml's proof job and port ALL deltas (incl. mailpit/SMTP + `kc_locale` realm wiring if they differ). Re-dispatch + job-verify = manager.
- [ ] **T-30** — (after T-29 re-dispatch) full-suite-only residuals: showcase-dashboard spec↔seed reconciliation (pending-validation 10≠4, sysadmin flights 12<14, pilot last-flight card absent — query the REAL seeded state, J-7 T-18 precedent, don't hand-patch constants); planning-notification mailpit duplicate (2 mails to club address, expected 1) + register verification-mail timeout IF they persist post-T-29.
