...
- [ ] **T-17** — GALLERY-SIMPLIFY fanout gap (gate-revealed): drop the fanout's stale `legacy-parity` sub-path deploy + its `[deployed-journey]` link-check (`alpenflight-proof-fanout.yml`) — superseded by the one-page model; J-11 has no legacy pairing so it's a thin page that reds the fanout.
