...
- **`PersonClub` PK + key strategy:** surrogate UUID v7 at ingest; mapper `foreignKeys()` declares PERSON + CLUB + MEMBER_STATE. No `legacy_id_map_person_club` — PersonClub is a leaf junction (no inbound FKs from other identity entities). PersonClub's `is_*` operational role flags port verbatim from legacy `PersonClub.Is*` (per-Club semantics); legacy `Person.Has*Licence` lives on `t_person.has_*_licence` (person-wide semantics) — two distinct surfaces, do not collapse.
