...
review_pass: 2  # second pass — post-rework + ADR 0022 merge guinea-pig
review_outcome: improvements-only
review_blockers: 0
review_improvements: 16  # 6 maintainability + 6 security + 4 code-quality
review_nudges: 4  # 2 security + 2 code-quality
...
review_directive_test_outcome: "ADR-0022 directives surfaced V4's Directive-2 violations as a coherent class (not 18 separate findings); reviewers correctly applied the security defense-in-depth carve-out + flagged S-132 scope-refinement gap; doc-drift correctly defaulted to improvement/nudge per Directive 1."
reworked: true
reworked_at: 2026-05-17
rework_mode: bold
rework_address_now: 13
rework_deferred: 0
rework_accepted: 8
rework_auto_decisions: 14
rework_followups: []
rework_meta_improvements: 3
rework_meta_followups: []
rework_meta_changes:    # folded into THIS PR (boyscout — never separate chore/* branches per memory rule)
  - { kind: conventions-update, target: "next/server/CONVENTIONS.md (new section: partial-predicate-on-soft-delete-tables)" }
  - { kind: workflow-improvement, target: ".claude/skills/modernize-implement/SKILL.md Step 6.8 (sweep checklist: stale test-method-name refs in story body)" }
  - { kind: workflow-improvement, target: ".claude/agents/security-engineer.md (require explicit prefer-X / deferred-to clause on every OR-option in threat-mitigation matrix)" }
---
...
**Reviewed:** 2026-05-17 (second pass — post-rework + ADR 0022 merge) · **PR:** #43 · **Diff size:** 10 commits, 31 files (+3746 / -2062 — includes the bundled `chore/modernize-primary-directives` merge) · **Outcome:** improvements-only

> **Second pass — guinea-pig test of ADR 0022 directives + trimmed reviewer agents against S-014's existing schema.** The new reviewers correctly surfaced S-014's 18 schema-level CHECK constraints + 1 generated calculation column as a coherent Directive-2 violation class (not 18 separate findings), recognised that S-132 (V5 follow-up) already owns the cleanup, and identified a real S-132 scope-refinement gap (OR Art. 957a defense-in-depth CHECKs currently listed under "Drops" should be "Retain"). Directive 1 correctly downgraded all remaining doc-drift findings to improvement / nudge.
>
> First-pass findings are replaced atomically per the skill's idempotency rule. The first-pass operator decisions remain reflected in the `## Rework notes` section + the in-place edits to V4 / tests / story body — read those for resolution history. The bullets below are NEW findings on top of the post-rework state.

### Maintainability
- **[improvement]** 18 business-logic CHECK constraints in V4 violate [ADR 0022 directive 2](../adrs/0022-modernization-primary-directives.md) — `next/server/src/main/resources/db/migration/V4__reservations_planning_accounting.sql:231, 264, 286, 385, 437, 439, 441, 447, 449, 451, 505, 507, 509, 511, 586, 588, 590, 592, 610`. State-machine value sets (`ck_dlv_process_state_in_set`), `Booked-requires-*` state-machine guards, value-object ranges (`>= 0`, `BETWEEN 0 AND 100`), `ck_arv_max_30_days` sanity cap, `ck_pln_planning_date_reasonable` belong on aggregate methods + value-object constructors at S-022/S-064; co-located CHECK + domain method is a dual-source-of-truth that drifts. **Fix:** addressed by S-132 (V5 drop); confirm V5 deletes all 18 plus the `ck_dlv_booked_requires_*` family that the legal-record carve-out covers in domain instead (see Security finding below — Booked-requires-* should be Retain).
- **[improvement]** `delivery_item.total_amount` generated column computes a domain value in the schema — `V4__reservations_planning_accounting.sql:491-492`. `quantity * unit_price * (100 - discount) / 100` is invoice math, not a structural invariant; per Directive 2 "Moves out of the schema" list. **Fix:** addressed by S-132; ensure V5 drops the column AND re-creates `ix_dli_delivery` without the `INCLUDE (... total_amount)` covering clause (see next finding).
- **[improvement]** `ix_dli_delivery` INCLUDE clause references a column S-132 will drop — `V4__reservations_planning_accounting.sql:514-516`. The covering index includes `total_amount`; when S-132 drops the column, this index needs re-creating. Easy to miss because the dependency isn't on the constraint list. **Fix:** flag in S-132 task list — V5 must `DROP INDEX ix_dli_delivery; CREATE INDEX ix_dli_delivery ON delivery_item(delivery_id) INCLUDE (article_id, article_number, quantity, unit_price);`.
- **[improvement]** Test class pins 18 directive-violating CHECKs as immovable behaviour — `next/server/src/test/java/ch/alpenflight/server/migration/ReservationsBaselineIntegrationTest.java:243-289, 517-544`, etc. Tests like `delivery_process_state_id_999_rejected_by_check` assert SQLSTATE 23514 at insert time; they freeze schema-side enforcement of a rule the aggregate will own. When S-132 drops the CHECK, the `_rejected_by_check` test breaks — and the aggregate-method test at S-022 is what should be asserting the rejection. **Fix:** S-132 task list already mentions these deletions; ensure parallel aggregate-method unit tests at S-022 land **before** V5 to avoid a coverage gap window.
- **[improvement]** `ck_dlv_booked_requires_*` retention argument under-specified — `V4__reservations_planning_accounting.sql:447-453`. ADR 0022's defense-in-depth carve-out covers OR Art. 957a legal-record invariants — the three Booked-requires-* CHECKs plausibly qualify (they protect the on-disk legal record, not just the booking transaction). But neither the column comment nor a constraint comment cites OR Art. 957a as the rationale. A future reader at S-132 refinement has to guess. **Fix:** add `COMMENT ON CONSTRAINT ck_dlv_booked_requires_recipient ... IS 'OR Art. 957a defense-in-depth — see ADR 0022';` on the three Booked-requires-* CHECKs, OR explicitly drop in S-132 with the domain method owning the rule alone.
- **[improvement]** `forbidden-migration-patterns.txt` doesn't yet block re-introducing business-logic CHECKs — `next/server/src/test/resources/security/forbidden-migration-patterns.txt:1-43`. ADR 0022 §"Risks" promises a CHECK deny-pattern in "the next sweep"; S-132 acceptance carries it, but until V5 lands, V6+ can silently regress. Two stories drift apart. **Fix:** acceptable to defer per S-132 ownership; nudge S-132 refinement to land the deny pattern in the same PR as the V5 drop so the gate exists from the moment the rules leave the schema.
...
### Security
- **[improvement]** S-132 acceptance lists `ck_dlv_booked_requires_number` / `_delivered_on` / `_recipient` under "Specific drops" — `docs/modernization/stories/S-132-v5-drop-business-logic-checks.md:9`. These three CHECKs are exactly the ADR 0022 §"Exception — invariants the legal record requires at all times" carve-out for OR Art. 957a frozen-invoice columns: a Booked row missing `delivery_number` / `delivered_on` / recipient snapshot is a Swiss-tax-law-noncompliant legal record, not a domain rule. The "Retain" bullet at L10 mentions `ck_dlv_process_state` (which is actually a state-machine value-set check, less load-bearing) but is silent on the three Booked-requires-* CHECKs. **Fix:** amend S-132 acceptance to explicitly list `ck_dlv_booked_requires_number` + `ck_dlv_booked_requires_delivered_on` + `ck_dlv_booked_requires_recipient` under **Retain** (defense-in-depth per ADR 0022 OR 957a carve-out), and move `ck_dlv_process_state_in_set` to **Drop** unless it earns a separate retention argument.
- **[improvement]** S-132 acceptance lists `ck_dlv_delivery_number_positive` under "Specific drops" — `docs/modernization/stories/S-132-v5-drop-business-logic-checks.md:9`. This isn't a value-object range check in spirit — it's the gap-free numbering integrity guard (`delivery_number` must be > 0 for the partial UNIQUE to mean what it claims; a `0` value would create an out-of-band invoice number). Belongs in the OR 957a defense-in-depth carve-out. **Fix:** move to Retain in S-132 acceptance or document why the partial-UNIQUE alone is structurally sufficient.
- **[improvement]** `ck_dlv_booked_requires_recipient` enforces only firstname + lastname; country / city / zip can be NULL on Booked — `V4__reservations_planning_accounting.sql:451-453, 454-457`. First-pass review accepted at "S-064 service layer per-club policy"; second-pass concern is that if S-064 doesn't ship the per-club policy-driven address-completeness gate **before any production invoice is booked**, the legal-record invariant has neither schema nor service-layer enforcement. **Fix:** raise the S-064 acceptance criterion to explicitly list "address-tuple completeness on Booked, per per-club policy" as a hard pre-prod requirement (not a deferred polish item).
- **[improvement]** `tenant-rules.yaml.AircraftReservations.pii_ride_through` includes `aircraft_id` despite Aircraft carrying no direct PII — `next/database/tenant-rules.yaml:415`. First-pass nudge upgraded by second-pass concern: the overloaded key feeds S-027 audit-blob redaction and S-051 DSAR scope — a downstream consumer treating `pii_ride_through` as "PII surface to redact" will either redact a non-PII identifier (audit forensics holes) or follow the FK to scrub Aircraft data on a Person DSAR (wrong subject). **Fix:** split into `pii_ride_through` (Persons, real PII) and `cross_tenant_fks` (Aircraft, authorization-relevant but not PII); update consumer-facing comment in the YAML.
- **[improvement]** `forbidden-migration-patterns.txt` does not yet carry the ADR 0022 CHECK-pattern denylist — `next/server/src/test/resources/security/forbidden-migration-patterns.txt:1-43`. S-132 acceptance L12 promises this for V5; until S-132 lands, V6+ implementers can re-add business-logic CHECKs ADR 0022 forbids. **Fix:** confirm S-132 lands the denylist in the **same** PR as the V5 drop + add a `# TODO(S-132): CHECK \(.+\) denylist with documented exceptions per ADR 0022` marker now.
- **[improvement]** S-024 hand-off for `aircraft_reservation.aircraft_id` documented but not yet enforced — `V4__reservations_planning_accounting.sql:633` + `next/database/tenant-rules.yaml:419`. Until S-024 parameterises its leakage CI over the new cross-tenant FK, the 2026-05-16 amendment lives only in column comments + YAML preconditions — no automated test. **Fix:** confirm S-024's story file lists `aircraft_reservation.aircraft_id` in its cross-tenant FK roster ground truth before S-024 enters refinement.
- **[nudge]** `COMMENT ON COLUMN delivery.flight_id` says "service layer (S-022)" but `aircraft_reservation.aircraft_id` says "(S-026/S-064)" — `V4__reservations_planning_accounting.sql:652, 633`. First-pass nudge unresolved; the inconsistency persists across the post-rework state. **Fix:** confirm whether S-022 or S-026 owns same-tenant flight-id assertion + harmonise.
- **[nudge]** `expected_matched_filter_ids BIGINT[]` is documented as "NOT FK-enforced — deleted filter is legitimate regression signal" but the type is mismatched (filter IDs are UUID per ADR 0019; this is BIGINT) — `V4__reservations_planning_accounting.sql:686-687`. The array references `accounting_rule_filter.legacy_int_id`, not `.id`. A future implementer might write `WHERE id = ANY(expected_matched_filter_ids)` and silently get zero rows. **Fix:** add a clarifying clause to the comment ("references `accounting_rule_filter.legacy_int_id`, not `.id`; intentional for S-016 legacy-test-data import").

### Code quality
- **[improvement]** CONVENTIONS.md `ix_arv_pilot` canonical-example line citation is wrong by 3 lines — `next/server/CONVENTIONS.md:105`. The cited range `V4__reservations_planning_accounting.sql:237-239` lands on `ix_arv_club_start_end`; `ix_arv_pilot` is actually at `:240-242`. A contributor navigating there to understand the partial-predicate convention lands on the wrong index. **Fix:** update CONVENTIONS.md line 105 to cite `:240-242`.
- **[improvement]** CONVENTIONS.md `ix_pda_planning_day` example line citation is wrong — `next/server/CONVENTIONS.md:101`. The cited range `V4__reservations_planning_accounting.sql:312-313` lands inside the `planning_day_assignment` FK constraint block; the `-- covers tombstones:` comment + `CREATE INDEX ix_pda_planning_day` are at `:315-318`. **Fix:** update CONVENTIONS.md line 101 to cite `:315-318`.
- **[improvement]** CONVENTIONS.md `ix_dcti_test` example line citation is wrong — `next/server/CONVENTIONS.md:102`. The cited range `V4__reservations_planning_accounting.sql:573-575` lands in the middle of the `delivery_creation_test_item` column body; the comment + `CREATE INDEX ix_dcti_test` are at `:595-597`. **Fix:** update CONVENTIONS.md line 102 to cite `:595-597`.
- **[improvement]** `ix_arv_location` lacks the required `-- covers tombstones:` annotation per the new CONVENTIONS.md rule — `V4__reservations_planning_accounting.sql:243-244`. The first-pass review accepted the index-shape deferral to S-108 (M#7), and CONVENTIONS.md line 107 documents this case as deliberate-tombstone-coverage — but the file itself has no `-- covers tombstones: deferred-perf-tuning S-108` comment. A future contributor sees an unexplained non-partial index. **Fix:** add `-- covers tombstones: deferred-perf-tuning S-108 — full-range scan benchmarked acceptable at current scale; revisit when per-club reservation count exceeds target` immediately above line 243.
- **[nudge]** `ReservationsBaselineIntegrationTest` wrapper-comment slightly misleads on future usage — `next/server/src/test/java/ch/alpenflight/server/migration/ReservationsBaselineIntegrationTest.java:1014-1016`. The comment says "Future migration stories should call the static helpers directly" — accurate, but the wrappers themselves are thin one-liners that stay in sync automatically; the framing slightly overstates the maintenance concern. **Fix:** harmless; tighten when the file is next touched.
- **[nudge]** `MigrationAssertions.assertFkDeleteRule` Javadoc parenthetical is long — `next/server/src/test/java/ch/alpenflight/server/testsupport/MigrationAssertions.java:133-139`. The "brittle across Postgres versions when position-in-unique-constraint behaviour differs" explanation is technically correct but spills across 3 lines where one would suffice. **Fix:** tighten to one sentence on next touch.

### Cross-reviewer agreements
- **S-132 scope needs refinement before V5 ships** — maintainability + security both flagged: the three `ck_dlv_booked_requires_*` CHECKs (Booked-requires-number / -delivered_on / -recipient) are OR Art. 957a defense-in-depth that should be **Retain** in S-132's acceptance, not Drop. Maintainability also flags `ck_dlv_delivery_number_positive` (gap-free integrity) for the same Retain treatment. **Highest signal — S-132 must be refined before it's worth running.**
- **Schema-level rules need parallel aggregate-method tests before V5** — maintainability flagged this as test-coverage continuity; security implicitly endorses (OR 957a invariants can't have a gap-week). **Highest signal — coordinate S-132 + S-022 sequencing.**
- **CONVENTIONS.md line-citation drift** — code-quality + maintainability both touch on this implicitly (maintainability validated the new partial-predicate rule's correctness; code-quality caught that 3 of its canonical-example line citations are wrong). **The new conventions doc undermines its own credibility at the moment it's being established.**
