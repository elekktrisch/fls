...
### Maintainability
- **[improvement]** [accepted: addressed by S-132; the V5 follow-up owns the full drop, including the Booked-requires-* family per operator policy "Directive 2 trumps in-DB security defense-in-depth" (2026-05-17)] 18 business-logic CHECK constraints in V4 violate [ADR 0022 directive 2](../adrs/0022-modernization-primary-directives.md) — `next/server/src/main/resources/db/migration/V4__reservations_planning_accounting.sql:231, 264, 286, 385, 437, 439, 441, 447, 449, 451, 505, 507, 509, 511, 586, 588, 590, 592, 610`. State-machine value sets, Booked-requires-* guards, value-object ranges, sanity caps belong on aggregate methods + value-object constructors at S-022/S-064. **Fix:** addressed by S-132 (V5 drop); aggregate-method enforcement lands at S-022/S-064.
- **[improvement]** [accepted: addressed by S-132 (drops the column at V5)] `delivery_item.total_amount` generated column computes a domain value in the schema — `V4__reservations_planning_accounting.sql:491-492`. **Fix:** addressed by S-132; coordinated with M3 below (V5 also re-creates the covering index).
- **[improvement]** [in-rework] `ix_dli_delivery` INCLUDE clause references a column S-132 will drop — `V4__reservations_planning_accounting.sql:514-516`. **Fix:** added to S-132 acceptance + task list (V5 must `DROP INDEX ix_dli_delivery; CREATE INDEX ix_dli_delivery ON delivery_item(delivery_id) INCLUDE (article_id, article_number, quantity, unit_price);`).
- **[improvement]** [accepted: hand-off to S-022 captured in Cross-story hand-offs section below; coordination requirement is structural so S-022 refinement picks it up when that story enters refine] Test class pins 18 directive-violating CHECKs as immovable behaviour — `ReservationsBaselineIntegrationTest.java:243-289, 517-544`, etc. **Fix:** aggregate-method unit tests at S-022 land BEFORE V5 to avoid a coverage gap window.
- **[improvement]** [accepted: moot per operator policy 2026-05-17 — Directive 2 trumps in-DB defense-in-depth; the three Booked-requires-* CHECKs are dropped at V5 per S-132 + the OR Art. 957a invariant is enforced solely by Delivery.book() aggregate method at S-064] `ck_dlv_booked_requires_*` retention argument under-specified — `V4__reservations_planning_accounting.sql:447-453`. **Fix:** drop at V5 per S-132; no `COMMENT ON CONSTRAINT` needed (constraint is going away).
- **[improvement]** [accepted: S-132 acceptance already lists `forbidden-migration-patterns.txt CHECK deny-pattern` as task; nothing to add at this story level] `forbidden-migration-patterns.txt` doesn't yet block re-introducing business-logic CHECKs — `next/server/src/test/resources/security/forbidden-migration-patterns.txt:1-43`. **Fix:** S-132 lands the denylist in the same PR as the V5 drop.
...
### Security
- **[improvement]** [accepted: operator policy 2026-05-17 — "Directive 2 (business logic in DDD domain) trumps in-DB security defense-in-depth." S-132's "Drops" list is correct; the OR Art. 957a invariant is enforced solely by `Delivery.book()` aggregate method at S-064. The security reviewer's RETAIN recommendation is overruled per the directive's purity.] S-132 acceptance lists `ck_dlv_booked_requires_number` / `_delivered_on` / `_recipient` under "Specific drops" — `docs/modernization/stories/S-132-v5-drop-business-logic-checks.md:9`. Reviewer flagged as defense-in-depth carve-out. **Fix:** drop per S-132 as written; aggregate method owns the invariant.
- **[improvement]** [accepted: operator policy 2026-05-17 — same rationale as above; the partial UNIQUE is structurally sufficient for the gap-free invariant. Application-layer numbering allocator at S-064 enforces `>= 1` for new rows; a `0` value can never enter via the allocator.] S-132 acceptance lists `ck_dlv_delivery_number_positive` under "Specific drops" — `docs/modernization/stories/S-132-v5-drop-business-logic-checks.md:9`. Reviewer flagged as gap-free integrity guard. **Fix:** drop per S-132 as written; partial UNIQUE + service-layer allocator cover the invariant.
- **[improvement]** [accepted: captured in `## Cross-story hand-offs` section below as a structural requirement on S-064 refinement; raised as a hard pre-prod requirement at that story's refine time] `ck_dlv_booked_requires_recipient` enforces only firstname + lastname; country / city / zip can be NULL on Booked — `V4__reservations_planning_accounting.sql:451-453`. **Fix:** S-064 acceptance lists "address-tuple completeness on Booked, per per-club policy" as a hard pre-prod requirement.
- **[improvement]** [in-rework] `tenant-rules.yaml.AircraftReservations.pii_ride_through` includes `aircraft_id` despite Aircraft carrying no direct PII — `next/database/tenant-rules.yaml:415`. Overloaded key feeds S-027 audit-blob redaction + S-051 DSAR scope — risk of audit forensics holes OR wrong-subject scrubbing. **Fix:** split into `pii_ride_through` (Persons) + `cross_tenant_fks` (Aircraft, authz-relevant non-PII); update consumer-facing comment.
- **[improvement]** [accepted: S-132 acceptance L12 already covers the deny-pattern (lands in the same PR as the V5 drop). No marker-comment needed in V4 today.] `forbidden-migration-patterns.txt` does not yet carry the ADR 0022 CHECK-pattern denylist — `next/server/src/test/resources/security/forbidden-migration-patterns.txt:1-43`. **Fix:** S-132 lands the denylist in the same PR as the V5 drop.
- **[improvement]** [accepted: captured in `## Cross-story hand-offs` section below; S-024 refinement will pick it up. Tracking the hand-off via column comment + tenant-rules YAML precondition is the deliberate first-step pattern (same shape worked for `flight.operating_club_id` cross-tenant invariant in S-013).] S-024 hand-off for `aircraft_reservation.aircraft_id` documented but not yet enforced — `V4__reservations_planning_accounting.sql:633` + `next/database/tenant-rules.yaml:419`. **Fix:** S-024 refinement adds `aircraft_reservation.aircraft_id` to the cross-tenant FK roster ground truth.
- **[nudge]** [auto-accepted: nudge severity, no contract impact; harmonise during S-022 / S-026 wiring when ownership crystallises] `COMMENT ON COLUMN delivery.flight_id` says "service layer (S-022)" but `aircraft_reservation.aircraft_id` says "(S-026/S-064)" — `V4__reservations_planning_accounting.sql:652, 633`. **Fix:** confirm + harmonise during S-022 / S-026 wiring.
- **[nudge]** [in-rework] `expected_matched_filter_ids BIGINT[]` is documented as "NOT FK-enforced" but the type references `accounting_rule_filter.legacy_int_id`, not `.id` — `V4__reservations_planning_accounting.sql:686-687`. **Fix:** clarify the comment.

### Code quality
- **[improvement]** [in-rework] CONVENTIONS.md `ix_arv_pilot` canonical-example line citation is wrong by 3 lines — `next/server/CONVENTIONS.md:105`. **Fix:** update to `:240-242`.
- **[improvement]** [in-rework] CONVENTIONS.md `ix_pda_planning_day` example line citation is wrong — `next/server/CONVENTIONS.md:101`. **Fix:** update to `:315-318`.
- **[improvement]** [in-rework] CONVENTIONS.md `ix_dcti_test` example line citation is wrong — `next/server/CONVENTIONS.md:102`. **Fix:** update to `:595-597`.
- **[improvement]** [in-rework] `ix_arv_location` lacks the required `-- covers tombstones:` annotation per the new CONVENTIONS.md rule — `V4__reservations_planning_accounting.sql:243-244`. **Fix:** add `-- covers tombstones: deferred-perf-tuning S-108 — full-range scan benchmarked acceptable at current scale; revisit when per-club reservation count exceeds target` immediately above line 243.
- **[nudge]** [auto-accepted: nudge severity, no contract impact] `ReservationsBaselineIntegrationTest` wrapper-comment slightly misleads on future usage — `ReservationsBaselineIntegrationTest.java:1014-1016`. **Fix:** harmless; tighten when the file is next touched.
- **[nudge]** [auto-accepted: nudge severity, no contract impact] `MigrationAssertions.assertFkDeleteRule` Javadoc parenthetical is long — `MigrationAssertions.java:133-139`. **Fix:** tighten on next touch.
