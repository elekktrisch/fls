---
id: S-152
title: Rename `alpenflight/` → `alpenflight/` working subtree
epic: E-01
status: in_progress
started_at: 2026-05-19
depends_on: []
acceptance:
  - All `alpenflight/server/`, `alpenflight/web/`, `alpenflight/database/`, `alpenflight/auth/`, `alpenflight/ops/` folders renamed to `alpenflight/...` in a single atomic commit.
  ...
  - Anchored grep `git grep -nE '(^|[^A-Za-z0-9_/])alpenflight/(auth|database|ops|server|web)' -- ':!flsserver' ':!flsweb' ':!e2e/node_modules' ':!docs/modernization/stories/implemented'` returns zero hits.
estimate: S
adr_refs: []
parity_test: none
refined: true
...
## Context
Vision-doc §8 final naming open item. The technical rebrand (S-128) covered the user-facing name but the working subtree slug is still `alpenflight/`. This story closes the loop.

## Acceptance criteria
See frontmatter.

## Tasks
- [ ] `git mv alpenflight/{auth,database,ops,server,web} alpenflight/...` — five renames, one commit.
- [ ] Anchored search-replace `\bnext/` → `alpenflight/` (see Design notes §2 for safe regex + exclusion globs).
- [ ] Update path filters in `.github/workflows/{ci,compose-lint,compose-smoke,extract}.yml` from `alpenflight/**` → `alpenflight/**` in the same commit.
- [ ] Update `.idea/gradle.xml` and any tracked `.idea/modules/*.iml` referencing `$PROJECT_DIR$/alpenflight/...`.
...
Anchored regex on `\bnext/` to spare prose ("next quarter", "next release", `alpenflight/last`, `nextTick`). Recommended one-liner:
...
Then eyeball `git diff` for any false-positive in backtick prose. **Do not** use plain `sed s/alpenflight/alpenflight/g` — ADR / CHANGELOG prose will detonate.

### 3. In-tree vs. out-of-tree split
**In-tree, this commit:** repo files only — workflows, `docker-compose.yml`, root `CLAUDE.md` / `README.md`, `.claude/**`, `docs/**` (excluding `implemented/`), all moved-into-`alpenflight/` files, tracked `.idea/` entries with `$PROJECT_DIR$/alpenflight/...`.
...
- Filesystem rename `/home/agent/fls-build/alpenflight/` → conditional on §10 open question.
- `.idea/workspace.xml` — per-user churn; drop if tracked.

### 4. Symlink decision (`alpenflight/web/node_modules`)
...
1. `mv /home/agent/fls-build/alpenflight /home/agent/fls-build/alpenflight-web`
...
GH Actions `paths:` filter evaluates against **changed files**, which on a rename PR includes BOTH source and destination paths — so the rename PR's own CI **does** trigger under the old `alpenflight/**` filter. *Post-merge*, future PRs touching `alpenflight/**` only would be skipped under a stale filter. Therefore update filters to `paths: 'alpenflight/**'` (no union needed — nothing under `alpenflight/**` exists after merge) in the same commit. **Verification gate:** inspect the PR's Checks tab; "skipped" is the silent-failure mode, not "passed".

### 6. `implemented/` story audit trail
**Don't rewrite paths inside `docs/modernization/stories/implemented/*.md`.** Those are historical records; the file was created at `alpenflight/...` and the citation is accurate-as-of-implementation. Excluded in the rg glob above.

### 7. What NOT to rename
- `flsserver/`, `flsweb/`, `e2e/node_modules/` — exclude via globs.
- Docker container/image names already `alpenflight-*` — no `alpenflight/` substring, unaffected.
...
- After merge, every in-flight `/modernize-fleet` worktree or unmerged story branch touching `alpenflight/` faces a mechanical rebase conflict on every moved file. Schedule for a fleet-quiet window (see §10).
- Future `/modernize-implement` / `/modernize-refine` invocations naturally use `alpenflight/` paths once main has merged.

## Edge cases & hidden requirements

- **Persistent env hazard (out-of-band):** `/etc/sandbox-persistent.sh:5` hardcodes `alpenflight` in `ESBUILD_BINARY_PATH`. Only matters if §10 decides to rename the build-cache dir; otherwise leave alone.
...
- **GHA cache key churn:** `ci.yml:108` hashes `alpenflight/web/pnpm-lock.yaml` — first post-rename run cold-misses cache (acceptable, note in PR).
...
1. **Angular project + build-cache rename.** Should `alpenflight` (Angular project name in `angular.json`, output dir `dist/alpenflight/browser/`, build cache dir `/home/agent/fls-build/alpenflight/`, `ESBUILD_BINARY_PATH`) also become `alpenflight-web` in this commit? Ripples to: Caddy SPA-fallback config (S-041), `alpenflight/web/CLAUDE.md` §9 docs, persistent env, contributor onboarding. **Defensible either way** — pick before starting:
...
2. **In-flight branch policy.** PR #80 (S-047) is open; S-151 / S-153 / S-157 / S-158 are unrefined-todo and will touch `alpenflight/` once started.
