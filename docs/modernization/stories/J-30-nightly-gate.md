...
- [x] T-17 — Legacy retries 2→3 (operator 2026-07-22). All 12 genuine legacy reds are fixed (T-04/T-12/T-13/T-14/T-15/T-16 → 155 pass / 0 genuine bugs); the residual is IRREDUCIBLE roaming Mono/AngularJS readiness flakiness (the failing spec dies at a different precondition each retry + roams to a different spec each run — verified across 7 runs). Operator chose retries 3 + accept flaky-recovered: the job reds only on `pw.outcome`, so 3 retries keeps the reference-only pool reliably green while the flaky count is still surfaced loudly in the T-08 tally; a genuine break still fails all 4 attempts. `playwright.config.ts:144`.

- [x] T-18 — Harden the chronic `flights-parity-J2` spec (run 29895525873: list `:139` on attempt 1, tow-form `:276` `element(s) not found` on retries 1-3). EMPIRICAL root cause of `:276`: the tow div `ng-if="needsTowplane(flightDetails.StartType)"` (flight-edit-tow-form.html:2) exists ONLY when `flightDetails.StartType==1`; `flightDetails` binds in `mapFlightToForm` (FlightsController.js:317) AFTER masterdata + the flight load resolve. The old preconditions (busy-clear + `#FlightDate input` value) do NOT prove that binding — the date-picker isolate-scope `$watch('ngModel')` paints the input on its own digest tick and `FlightDate` has a `|| new Date()` fallback (FlightsController.js:335), so the date populates while `StartType` is still undefined → `needsTowplane(undefined)` false → the ng-if div is absent for the whole 15s. HB-3407 IS genuinely aerotow (`@startTypeId=1`, seed "6 Insert Test Flights.sql":54) — no expectation change. Fix: (1) list load polls the SPECIFIC HB-3407 row count (not any row); (2) after click, gate the hash on a real `/flights/<GUID>` (rules out list/`new`); (3) NEW ROOT-CAUSE GATE — poll the controller scope `flightDetails.StartType===1` (TEST_WRITING.md §6 pattern) before the tow div, the load-timing truth; (4) motor list polls row count. No parity assertion weakened. tsc clean, `--list` discovers. Confidence: high for `:276` (deterministic scope gate replaces the blind proxy) and for the list/motor races (getData re-fire poll); residual risk is only Mono/AngularJS backend-readiness timeouts common to all legacy specs — if J2 still exhausts retries next run it is irreducibly flaky → quarantine per the operator's residual-flake path.
- [x] T-19 — Quarantine `flights-parity-J2` (fix-before-quarantine exhausted). Run 29896644096 with the hardened J2 (retries:3): 151 pass / 1 fail / 5 flaky — J2 STILL exhausted all 4 attempts (`:142` HB-3407 row never rendered / `:314` T-18's `StartType===1` scope gate never satisfied — the legacy list + edit-form bind genuinely never arrive under CI load), while the 5 flaky (auth smoke + login) all recovered on retry. J2 is irreducibly flaky on the Mono/AngularJS reference stack — the sole hard fail after all 12 genuine reds were fixed. Quarantined via `@quarantine-legacy` tag (`flights-parity-J2.spec.ts:98`, why-commented) + `--grep-invert @quarantine-legacy` in `nightly.yml` (why-commented) + fix-owner rider `_BOYSCOUT [LEGACY-J2-READINESS]`. Explicit + documented exclusion (mirrors the real-idp KC-26 pattern), never a silent drop.
