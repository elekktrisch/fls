...
- [ ] T-16 — Legacy: 2 concrete bugs (NOT a cap, NOT flakiness — triage: 154 pass / 2 fail / 1 flaky, 8.0m). (a) `flights-parity-J2.spec.ts:166` — T-15's explicit-row selector has a broken CSS (missing close-paren → "Unexpected token", fails ×3 fast): close the paren. (b) `flight-reports.spec.ts:162` — REAL data gap: the self-seeded glider is genuinely absent from the per-group totals (90s poll exhausts ×3 — data, not slowness); the "today/glider/LSZK/this-year → must count" reasoning was ANALYTICAL and the gate REFUTED it ([[feedback_empirical_over_analytical_enumeration]]). EMPIRICALLY trace (download the run trace/artifacts for the report's actual response + read `ensureGliderFlight` vs the report's grouping/location/validity filter): is the flight's location ≠ the report's location filter, is self-launch excluded, is it un-landed/invalid, or a date/tenant mismatch? Fix the RIGHT thing (seed a countable flight / correct the report nav-params / correct the parity expectation) without weakening. Right-size the poll/test timeout (data, not slow → drop the 240s latent-bomb; the flight-reports 90s×retries×240s risks the 10-min cap).
