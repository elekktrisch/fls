...
- [ ] T-09 — Fix the audit spec real-idp red (Pillar 1). `audit-log-two-club.spec.ts:388` fails in the real-idp nightly: the Aircraft row is NOT visible under a `targetEntityType=Aircraft` filter (positive control) — fails identically on main (the pre-existing J-13 audit-filter bug the adversarial spec surfaced). The T-03 backend IT PASSES (predicate excludes correctly at IT level), so the cause is NOT a dropped `cb.equal` — reproduce locally (real-idp, LAN PG), find the true cause (targetEntityType naming mismatch aircraft-CREATE-stores vs the filter literal / free-text filter timing / create-not-audited), fix it, re-run the spec green.
