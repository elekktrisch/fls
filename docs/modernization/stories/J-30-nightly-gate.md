...
- [ ] T-18 — Harden the chronic `flights-parity-J2` spec (last legacy red; fix-before-quarantine). J2 is the heaviest legacy spec (glider list → flight-edit form → tow-form → motor air-movements) and hard-fails every run, racing at a different step each time (list `:130/:139`, tow-form `:276`) — retries:3 didn't rescue it. EMPIRICALLY diagnose the `:276` tow-form miss first (from the run trace): does T-16's `has:`/`hasText:` locator actually OPEN the HB-3407 aerotow row's edit form, and is `flightDetails.StartType==1` loaded when `needsTowplane` evaluates (vs the assertion racing the async edit-form fetch → `needsTowplane(undefined)` → no tow-form)? Then make EVERY J2 step robust (wait-for-populated list, deterministic row-open, wait-for-flightDetails-loaded before the tow-form assertion, motor tab) so it stops racing at any step — without weakening parity. If a comprehensively-hardened J2 STILL exhausts retries next run, it is irreducibly flaky on this reference stack → quarantine it with a fix-owner rider (operator's residual-flake path) and let the other 155 specs go deterministically green.
