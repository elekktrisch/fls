...
- [ ] **T-11** — `/join/pending` screen: public club projection (name/city/logo) + Withdraw + SSE subscribe → on approved force OIDC token-refresh → `/start`, on denied show reason → `/join`, on withdrawn → `/join`; + the `/start` guard (no `t_user` + live request → `/join/pending`; neither → `/join`).
  - BLOCKED — backend gap: the pending page needs the requested club's name + city + logo for a pilot with NO tenant, but no source exists. `GET /api/v1/me/join-request` (`JoinRequestResponse`) returns only `clubId` — no display fields. `GET /api/v1/clubs/{id}` is `@TenantId`-gated (a tenant-less pilot can't read it; cross-tenant fetch is forbidden). No `public/clubs/{id}` endpoint exists. The `Club` aggregate has `clubname` but **no `city` and no `logo`/`logoUrl` column mapped** (legacy `t_club` has `City` but no logo at all — logo is net-new). Needs a backend task: add `city` + `logoUrl` to the `Club` aggregate (Flyway + mapper), then EITHER a public projection `GET /api/v1/public/clubs/{id}` gated by the caller holding a live `JoinRequest` for that `clubId`, OR fold name/city/logoUrl into `JoinRequestResponse`. Until then `/join/pending` can only render the submitted-at / note / withdraw, not the club display.
