...
depends_on: [S-016]
acceptance:
  - A new Gradle module under `next/migration-tool/` builds a single-file fat-jar (`alpenflight-export.jar`).
  ...
  - A README in `next/migration-tool/` documents the usage including a worked example for a typical FLS deployment.
estimate: L
adr_refs: [0001, 0019]
...
Sharing code with S-016: extract the schema-mapping into a `next/migration-bundle/` library consumed by both this JAR and the server-side ingest pipeline (S-141). One source of truth for "what's in the bundle".

## Acceptance criteria
See frontmatter.

## Tasks
- [ ] Bootstrap Gradle module `next/migration-tool/` with the Shadow plugin (fat-jar build).
