...
- **[KC-26 UPGRADE DRIFT] 2 cross-journey real-idp nightly reds (pre-existing, surfaced when J-26 T-03 re-enabled the 12-day-dead nightly).** [S2] NOT J-26's vertical (validation/JDBC) — KC-26-upgrade reconciliation that needs iterative live-stack debugging; J-26's OWN real-idp proof is green. (1) `login.spec.ts:92` `?ui_locales=fr` -> `<html lang="fr">` still renders `en` (KC 26 honors the param differently / login-theme or realm i18n-resolver — needs live-KC iteration, possibly a realm/theme config change); (2) `token-lifecycle.spec.ts:47` silent-refresh still red after the wait-hardening (likely real KC-26 refresh-grant/SSO behavior, not timing). Each fix -> ~25-min nightly dispatch -> observe -> repeat. Ride the next journey's gate (or a focused KC-26-reconciliation slice). **The third red, `register.spec.ts` verify-mail, was NEVER an SMTP or KC-26 fault and is fixed in J-19 T-11: the realm keeps username separate from email (`realm-export.json:2247`), so Keycloak's registration form requires `#username`, and `kc-form.ts` never filled it — the form was rejected, no user was created, and no mail was ever sent. `compose-smoke.yml` had proven KC->Mailpit delivery green in CI the whole time.** *(seam: realm-export.json i18n + login/token real-idp specs + KC 26 OIDC behavior)* [[project_real_idp_real_roles_catches_authz_gaps]]
