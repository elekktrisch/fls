...
- **[KC-SET-USER-ATTRIBUTE-PARTIAL-PUT]** [S1] `KeycloakDeploymentDirectory.setUserAttribute` sends a single-key
...
- **[KC-26 UPGRADE DRIFT] 2 cross-journey real-idp nightly reds (pre-existing, surfaced when J-26 T-03 re-enabled the 12-day-dead nightly).** [S2] NOT J-26's vertical (validation/JDBC) — KC-26-upgrade reconciliation that needs iterative live-stack debugging; J-26's OWN real-idp proof is green. (1) **`?ui_locales=fr` was NEVER a KC-26 fault and is fixed in J-19 T-12: the spec's raw authorize URL carried no PKCE parameters, which the `alpenflight-web` client requires (`realm-export.json:1745`), so Keycloak 302'd to the SPA with `error=invalid_request&error_description=Missing parameter: code_challenge_method` and the login page never rendered. The `en` came from the SPA, which writes `navigator.language` to `document.documentElement.lang` (`locale.service.ts:49`). A live 26.5 container renders `<html lang="fr">` from `ui_locales` alone, even against `Accept-Language: en-US`. `kc_locale` is inert on KC 26.5.**; (2) **silent refresh was NEVER a KC-26 refresh-grant or SSO fault and is fixed in J-19 T-13: the spec patched the realm `accessTokenLifespan` to 30 s, below the SPA's 60 s renew window (`auth.config.ts:20`). Keycloak gives the id token the same lifespan, so the OIDC client read the fresh id token as expired at the code-flow callback and looped between the SPA and the realm. A second fault was ours too: the client fires `NewAuthenticationResult` on every silent renew, and the bridge ran the post-login redirect for it, so each rotation moved the operator to `/start` (`oidc-session-bridge.ts:52`). A live Keycloak 26.5 run rotated the token three times and kept the session.** All three reds carried a wrong KC-26 diagnosis; this rider is discharged. **The third red, `register.spec.ts` verify-mail, was NEVER an SMTP or KC-26 fault and is fixed in J-19 T-11: the realm keeps username separate from email (`realm-export.json:2247`), so Keycloak's registration form requires `#username`, and `kc-form.ts` never filled it — the form was rejected, no user was created, and no mail was ever sent. `compose-smoke.yml` had proven KC->Mailpit delivery green in CI the whole time.** *(seam: realm-export.json i18n + login/token real-idp specs + KC 26 OIDC behavior)* [[project_real_idp_real_roles_catches_authz_gaps]]
