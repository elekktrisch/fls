...
- [ ] **T-13** — Public controllers + request/response DTOs + `@Operation(operationId=…)`: discovery `POST` + available-days `GET`, scenic `POST`. Server-side validation of the fields legacy only guarded client-side. Takes over `PublicRegistrationController`'s two body-less `202` POSTs (T-07 scaffolding: paths, anonymous access and the 404/403 contract are already settled + IT-covered) and returns `201` + `Location`. **Delete the two now-dead body-less `PublicRegistrationIntake` overloads (`:41,46`) once the registrant-carrying ones at `:50,56` are reachable over HTTP.** **Also: `acceptDiscovery`'s `selectedDay` is currently unvalidated (T-10) — it MUST be checked against the club's published `DiscoveryFlightDay` rows, or an anonymous caller books an arbitrary date and the day picker becomes decorative.** *(seam: two public controllers)*
