...
- [ ] **T-12a** — **Fix the 503 contract first, then build against it.** `DemoSessionController.java:37-39` declares no content for 503, so `openapi.json` types that body as `DemoSessionResponse`, but the real body is `ProblemDetail` (`DemoSessionExceptionHandler.java:33`). The seat-busy state is AC-8, so the screen must not read a wrong type. Regenerate the snapshot and the client.
- [ ] **T-12b** — **`/demo` replaces `DemoStubComponent`.** Start the session, land on `/start`, render the seat-busy state of AC-8. Update the cross-journey consumers of the stub: `landing.spec.ts:133` (asserts `demo-stub` visible) and `demo.routes.ts:6`.
- [ ] **T-12c** — **The demo banner across every demo screen (AC-3)** — its permanent text, its call-to-action to `/signup?intent=migrate`, and the funnel telemetry.
