...
8. **Test philosophy — integration-tests only, no mocking** (operator override mid-implement). The refinement's ~20-verifier bash corpus was first replaced with JUnit + Testcontainers; operator further simplified: "for tests, please only write integrationtests that connect to the actual mssql database, no need to set up any mocking." Concrete shape:
   - **One integration test class** (`MetadataExtractorIntegrationTest`) — spins up SQL Server via Testcontainers, applies synthetic DDL, runs the extractor end-to-end, asserts JSON outputs.
   ...
   - **No `DataSource` / `JdbcTemplate` mocking, no in-memory DB emulation.** The real `mcr.microsoft.com/mssql/server` image is the only test backend.
   - Skipping the unit tier means refactors are tested against real SQL Server behavior, not against test doubles that drift from production semantics. Slower per-run, simpler corpus, fewer false-confidence signals.

   The `FlsTestFixtureIntegrationTest` (run against the actual `flsserver/database/FLSTest/` fixture) is **also dropped from this story**. Reason: synthetic DDL exercises every output shape; the real FLSTest fixture adds nothing to that coverage that isn't observable from a single ad-hoc run. The operator runs it manually against the seeded container when they want real-shape JSON output; the integration test corpus stays focused.
