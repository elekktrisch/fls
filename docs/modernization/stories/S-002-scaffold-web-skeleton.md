...
**Reviewed:** 2026-05-15 · **PR:** #9 · **Diff size:** 5 commits, 52 files, +7317 / −9 · **Outcome:** blockers (3 blockers, 11 improvements, 5 nudges)

### Maintainability

- **[blocker]** [accepted: operator chose to update the design contract instead of renaming the project — the angular.json project stays `web` and `dist/web/browser/` is the canonical artifact path. S-041 + the perf-plan CI source-map check will be authored to match (`dist/web/browser/...`). M-I4 (README drift) dissolves: README is now correct under this amended contract.] Angular project named `"web"` contradicts the design contract `"next-web"` — `next/web/angular.json:9` declares `"projects": { "web": ... }`. Design notes pin the workspace project as `next-web` (line 60, 160); the S-041 Caddy deploy contract and the perf-plan source-map CI check both reference `dist/next-web/browser/`. The README at `next/web/README.md:44` already drifted to `dist/web/browser/` to match the wrong build output. Two downstream contracts (S-041 deploy, perf-plan CI) now point at a path the build doesn't produce. **Fix:** rename the angular.json project to `next-web` (update the `buildTarget` cross-refs in serve.configurations + the README artifact path).
- **[blocker]** [in-rework] `provideBrowserGlobalErrorListeners()` violates the explicit "skeleton commits NO `window.onerror` handler" rule — `next/web/src/app/app.config.ts:13`. The security plan repeats the prohibition three times (§337 "no `window.onerror`; S-034 owns telemetry", §372, §392 "A09: Skeleton does NOT commit `window.onerror` global"). The Angular 20+ `provideBrowserGlobalErrorListeners` provider is precisely the `window.onerror` + `window.onunhandledrejection` registrar S-034 was reserved to own. **Fix:** delete the import + provider call; let S-034 wire telemetry.
- **[blocker]** [in-rework] `AppComponent` shell does not implement the per-route nav-bar toggle promised "from day 1" — `next/web/src/app/app.ts:7` is `<router-outlet />` only. Design notes §89 specified `@if(showNavBar) { <fls-nav-bar/> } <router-outlet/>` and §241 / §308 explicitly call this the structural fix for the legacy `||` tautology bug at `flsweb/src/index.js:50`. The route shape carries `data.showNavBar` (`landing.routes.ts:7`) but no consumer in the shell reads it; every subsequent feature story (S-021, S-097, S-098, S-099) will either re-derive the pattern or skip it. **Cross-reviewer agreement:** flagged by both maintainability and usability. **Fix:** add a `showNavBar` signal in `App` derived from `Router.events.pipe(filter(NavigationEnd))` over deepest-route `data['showNavBar']`; render `@if (showNavBar()) { <!-- TODO(S-008): <fls-nav-bar/> --> }` above the outlet so the consumer pattern is on file even before the organism exists.
- **[improvement]** [in-rework] `styles.css` `@source "./**/*.{html,ts}";` is redundant and primes a future foot-gun — `next/web/src/styles.css:3`. Tailwind v4 auto-scans the entry CSS's directory and the project root; the directive only matters for out-of-tree sources. If someone later moves `styles.css` (e.g. into `src/styles/`), this glob silently narrows scanning instead of expanding it. **Fix:** delete the line; restore only when an out-of-tree `@source` is genuinely needed (per `CLAUDE.md §3`).
- **[improvement]** [in-rework] `.vscode/launch.json` still ships a Karma debug-URL config — `next/web/.vscode/launch.json:14` launches `http://localhost:9876/debug.html` for `ng test`; that's Karma's. Under Vitest there is no such page; the debugger fails silently for any teammate who uses VSCode's launch profile. **Fix:** drop the `ng test` entry or replace with a node-attach config for Vitest's `--inspect-brk`.
- **[improvement]** [in-rework] CI bakes the sandbox's `ignore-scripts=true` quirk into GitHub Actions — `.github/workflows/ci.yml:428,435` set `npm_config_ignore_scripts: 'true'` for install and build. The `package.json:23-29` `onlyBuiltDependencies` allowlist becomes inert on CI as a result. A non-sandbox CI runner has no reason to inherit the Windows-mount workaround; the next maintainer will wonder why the allowlist exists. **Fix:** drop the env var from the CI step (let pnpm honor `onlyBuiltDependencies` normally), or comment it with rationale + "remove when sandbox host is gone" TODO.
- **[improvement]** [accepted: dissolves under B1's contract-update decision — `dist/web/browser/` is now the canonical artifact path, README is correct as-is.] README artifact path drifted to match the wrong project name — `next/web/README.md:44` says `dist/web/browser/`. Whether fixed by renaming the angular project or by editing the README, the README must round-trip with the artifact path promised to S-041. **Fix:** align with the project-rename blocker above.
- **[nudge]** [accepted: mode-only diff with no content change; rolling it back would reintroduce the CI red. Acceptable scope leakage in a foundation PR.] `next/server/gradlew` mode flip (644 → 755) sneaked into this PR — out of scope for S-002. The diff is mode-only with no content change so harmless, but it muddles the commit's scope.
- **[nudge]** [deferred → S-127] `eslint.config.mjs` doesn't enforce `no-restricted-imports` for atomic-design layering or cross-feature imports — `CLAUDE.md §1` / §2 promise ESLint enforces "atoms import nothing from molecules/organisms" and "direct imports between feature folders are forbidden." The skeleton has no siblings yet so the rule isn't load-bearing today, but the convention claim is currently unenforced. Reasonable to defer to S-008 / first cross-feature story.

### Security

- **[improvement]** [in-rework] `no-restricted-syntax` selector for `bypassSecurityTrust*` only catches static-class access — `next/web/eslint.config.mjs:30`. Selector `MemberExpression[object.name='DomSanitizer'][property.name=/^bypassSecurityTrust/]` matches `DomSanitizer.bypassSecurityTrustHtml(...)` (static) but not the realistic pattern `this.sanitizer.bypassSecurityTrustHtml(...)` with `sanitizer = inject(DomSanitizer)`. The plan named this rule as the primary A03 mitigation. **Fix:** broaden to `CallExpression[callee.property.name=/^bypassSecurityTrust/]`.
- **[improvement]** [in-rework] `localStorage`/`sessionStorage` rule is narrower than the plan specified — `next/web/eslint.config.mjs:33`. Plan asked for `no-restricted-globals` (catches every reference); implementer used `no-restricted-syntax` matching only `*.setItem`. Misses `removeItem`, `clear`, bracket-notation writes, `window.localStorage.setItem(...)`, and any read. Plan rationale was the R10 calcification — token *reads* are as load-bearing as writes. **Cross-reviewer agreement:** flagged by both security and maintainability. **Fix:** add a `no-restricted-globals` block listing `localStorage` and `sessionStorage`, or broaden the syntax selector to catch any `Identifier[name=/^(local|session)Storage$/]`.
- **[improvement]** [in-rework] `@angular-eslint/template/no-bypass-trust` (named in the plan) is not present in the template rules — `next/web/eslint.config.mjs:41-49`. Only `template/no-any` is configured beyond the recommended/accessibility extends; the plan listed `no-bypass-trust` as an explicit rule. **Fix:** add `'@angular-eslint/template/no-bypass-trust': 'error'` to the HTML block (or verify it's enabled via `templateRecommended` and document that).
- **[improvement]** [in-rework] `ignore-scripts` is sandbox-global, not project-pinned — `next/web/.npmrc:1-2` only sets `node-linker` and `package-import-method`. A fresh clone on a new contributor's machine would run install scripts under default pnpm 11 unless they replicated the sandbox's global config. `pnpm-workspace.yaml:3-12` mitigates with the `onlyBuiltDependencies` allowlist (4 packages), and CI overrides via env var, so the supply-chain surface is bounded — but the project isn't self-defending. **Fix:** add `ignore-scripts=true` to `next/web/.npmrc` so the policy travels with the repo (A06 / A08 defense-in-depth).
- **[improvement]** [accepted: inheriting workflow-level defaults is acceptable for a build-only job in a small-team OSS repo; tightening per-job adds maintenance friction without proportional security gain. Revisit if multi-tenant CI runners become a thing.] `next-build` CI job has no `permissions:` block — `.github/workflows/ci.yml:362`. Inherits the workflow / repo default `GITHUB_TOKEN`, broader than a build-only job needs. `e2e` correctly scopes itself (`ci.yml:168-169`). **Fix:** add `permissions: { contents: read }` (plus `actions: read` if needed for cache).
- **[improvement]** [deferred → S-126] pnpm-store cache `restore-keys` allows cross-lockfile cache reuse — `.github/workflows/ci.yml:419-421`. The fallback `${{ runner.os }}-pnpm-store-` matches any prior store regardless of `pnpm-lock.yaml` hash; combined with `--frozen-lockfile` install still verifies, but pnpm's content-addressed store check is the only line of defense. **Fix:** drop the `restore-keys` for the pnpm store or tighten to a major-version-pinned scope.
- **[nudge]** [in-rework] CSP `style-src 'unsafe-inline'` is correctly documented as a dev-only concession — story explicitly accepts it and reserves prod-tightening for the reverse proxy (S-041). Track the S-041 follow-up to drop `'unsafe-inline'` once styles fully extract. **Updated decision:** tighten now (drop `'unsafe-inline'`) — operator accepts the dev-server cost.
- **[nudge]** [in-rework] `@source "./**/*.{html,ts}"` is broader than v4's default — not a security issue (Tailwind doesn't ship arbitrary file contents to the browser), but worth tightening per the design-notes guidance to enumerate specific paths. (Same line removed by M-I1 above.)

### Usability

- **[improvement]** [in-rework — same finding as B3 above] `App` shell ignores `data.showNavBar` despite the route already carrying it — `next/web/src/app/app.ts:7` / `landing.routes.ts:7`. **Cross-reviewer agreement:** see the maintainability blocker above for the consolidated finding.
- **[improvement]** [in-rework — rename app.ts → app.component.ts, class App → AppComponent] File-naming convention inconsistent between root shell and feature components — `next/web/src/app/app.ts:1` vs `next/web/src/app/features/landing/landing.component.ts:1`. CLAUDE.md §6 mandates `name.component.ts`; the design notes (story line 89) spell `app.component.ts`. The Angular CLI 21 default dropped the `.component` suffix; the impl kept the new default for `app.ts` but used `.component.ts` for `landing`. Future contributors pick whichever variant they see first — divergent muscle memory. **Fix:** rename to `app.component.ts` (and `App` → `AppComponent`) to match CLAUDE.md §6 and the design notes; or update CLAUDE.md §6 to allow the suffix-less default. One rule, both files conform.
- **[nudge]** [in-rework] Landing placeholder hardcodes English "Hello FLS" under `<html lang="de">` — `next/web/src/app/features/landing/landing.component.ts:5`. The story explicitly declares the landing component a throwaway scaffold smoke (replaced by S-097), and `public/i18n/de.json` is intentionally empty. Acceptable as scaffold, but a `// TODO(S-097): replace with i18n-keyed German content` comment prevents the placeholder being forgotten when grep'ing for hardcoded strings later.
- **[nudge]** [in-rework] `fls-landing` template has no `<main>` landmark — `next/web/src/app/features/landing/landing.component.ts:5`. Bare `<h1>` directly under `<router-outlet />` means screen readers land outside any landmark. Worth pinning the convention in CLAUDE.md or in `App`'s template so every feature renders inside a landmark by default rather than re-deciding per feature. **Decision:** wrap App template in `<main>` so every feature renders inside a landmark by default.
- **[nudge]** [in-rework — delete index.ts files, keep .gitkeep] `shared/ui/*/index.ts` files all just `export {};` — empty re-export barrels can mask circular-import / unused-export bugs once primitives land. CLAUDE.md §6 says imports go through `@ui/atoms/<thing>` not the barrel root — so the barrels may never carry meaningful exports. Either add a one-line comment documenting "intentionally empty — consumers import deep paths per CLAUDE.md §6", or drop them and rely on `.gitkeep` + the directory-level path alias.

### Cross-reviewer agreements

- **`AppComponent` shell missing `data.showNavBar` consumer** — maintainability flagged as blocker, usability as improvement. Synthesized as blocker per severity rubric: design notes §89/§241 name this as the structural fix for the legacy `||` tautology bug shipped "from day 1." Highest-signal finding.
- **ESLint `no-restricted-syntax` storage rule too narrow** (`setItem` only) — maintainability + security both flagged. The R10 calcification gate the security plan ordered is theatrical against the patterns real code uses.
