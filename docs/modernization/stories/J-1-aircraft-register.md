...
- [x] **T-15** — Gate-revealed (T-13's gate): the migrated-render test's new list-column assertions
...
- [x] **T-25** — Operator: the PR must show the **correct** proof URL. The J-25 sticky comment links the
  ...
  **Durable:** add a step to `alpenflight-proof-fanout.yml` that, after the branch-preview deploy, resolves the
  PR for `github.ref_name` (`gh pr list --head`) and **upserts** that sticky comment with the legacy-parity URL
  (needs `pull-requests: write`). So the fanout self-maintains the PR proof link for every journey. *(seam:
  fanout workflow PR-comment upsert step)* — e2e-driver.
