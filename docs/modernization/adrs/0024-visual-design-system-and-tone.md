...
- **Migration cost:** low. The existing `alpenflight/web/src/styles.css` already encodes most of this (slate-free at the moment, brand-500, Roboto, density tokens, ant-bridge). Concrete deltas: `--radius-md: 0`, add semantic surface tokens, add `tabular-nums` utility, install `lucide-angular`, wire `<af-icon>` to Lucide, swap any non-`stone`/`zinc` neutrals to slate.
...
- **Follow-ups (other stories implied):**
  - **Story (boyscout into next story PR per [[feedback-boyscout-rule-over-clean-prs]]):** update `alpenflight/web/src/styles.css` —
  ...
  - **Story:** install `lucide-angular`; wire `<af-icon>` atom to Lucide; document the convention in `alpenflight/web/CLAUDE.md`. The kit (S-008) ships every primitive's icon affordance pointing at Lucide.
