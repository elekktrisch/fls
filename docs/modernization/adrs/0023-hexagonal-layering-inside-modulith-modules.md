...
- **Follow-ups (other ADRs / stories implied):**
  - **Story:** reshape `next/server/src/main/java/ch/alpenflight/clubs/` into `clubs/{domain,application,web,infra}/`. `Club`, `MemberState`, `ClubRepository` (interface), `ClubNotFoundException`, `SlugAlreadyExistsException` move to `domain/` (the two exception types lose their `@ResponseStatus` annotation; a new `@RestControllerAdvice` in `web/` translates them). `ClubsService`, `ClubDtos`, `ClubMapper` move to `application/` (the DTO is the service's wire contract; mapper is the service's translator). `ClubsController` and the new exception handler land in `web/`. `ClubsRepository` renamed `JpaClubRepository`, extends both `JpaRepository<Club,UUID>` and `ClubRepository`, to `infra/`. Add `package-info.java` per package stating the dependency rule.
