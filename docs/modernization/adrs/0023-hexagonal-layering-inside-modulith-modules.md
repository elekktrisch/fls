...
[ADR 0018](0018-domain-model-ddd-aggregates.md) commits to DDD aggregates per bounded context with Spring Modulith and per-aggregate repositories, but does not lock the *layering inside a module*. The S-048 walking skeleton (`next/server/src/main/java/ch/alpenflight/clubs/`) demonstrates the gap: `Club` is both aggregate root and JPA entity, `ClubsRepository` is a Spring Data interface, `ClubsService` is `@Service`, and DTOs / controller / mapper / exceptions all live in the same flat package. Nothing structurally prevents a future controller from reaching past the service into the repository, a DTO from depending on the aggregate, Jackson annotations from creeping onto `Club`, or a domain rule from quietly acquiring a Spring dependency.
...
- **Capabilities:**
  - Each bounded context is a top-level package (`ch.alpenflight.<context>`), already its own Spring Modulith module per ADR 0018. Inside each module, four sub-packages with enforced dependency direction:
    - `domain/` — aggregate roots, value objects, domain services, domain events, repository **interfaces**. **JPA annotations allowed on aggregates** (Vernon-style pragmatic Hibernate-DDD); banned: Spring (except `@DomainEvents`), web stack, Jackson, JDBC, file IO, external clients, `@Component`-family.
    - `application/` — use-case services (`@Service`, `@Transactional`), orchestration of domain calls. Depends on own `domain/` + published interfaces of other modules.
    - `web/` — controllers, request/response DTOs, mappers. Depends on own `application/`. May NOT touch own `domain/` or `infra/` directly.
  ...
  - Enforcement in CI: Spring Modulith's `ApplicationModules.verify()` for inter-module boundaries + ArchUnit rules for inner-layer direction (no `web` → `domain`, no `domain` → `Spring*`, no `*` → another module's `domain/infra/web`).
...
- **Migration cost:** small. S-048's `clubs/` reshapes into `clubs/{domain,application,web,infra}/` (~30 min, one PR, no business-rule changes). Subsequent stories (S-022 entity plumbing, S-058 flight validator port, S-062a flight CRUD, etc.) inherit the template — per-module overhead is three extra `package-info.java` files and a tiny `JpaXxxRepository` extending the domain interface.
...
- **Follow-ups (other ADRs / stories implied):**
  - **Story (boyscout in next implementing PR, per `[[feedback-meta-improvements-are-boyscout]]`):** reshape `next/server/src/main/java/ch/alpenflight/clubs/` into `clubs/{domain,application,web,infra}/`. `Club`, `ClubRepository` (interface), `MemberState` move to `domain/`. `ClubsService` to `application/`. `ClubsController`, `ClubDtos`, `ClubMapper`, `ClubNotFoundException`, `SlugAlreadyExistsException` to `web/`. `ClubsRepository` renamed `JpaClubRepository`, extends both `JpaRepository<Club,UUID>` and `ClubRepository`, to `infra/`. Add `package-info.java` per package stating the dependency rule.
  - **Story:** add Spring Modulith dependency + `ApplicationModules.verify()` test to `next/server/`. Land before S-022 entity plumbing so the next ten stories inherit the verification. Implied by ADR 0018 follow-ups; this ADR ratifies the trigger.
  - **Story:** add ArchUnit dependency + rule suite to `next/server/src/test/java/ch/alpenflight/arch/`. Rules: (1) `domain/` may not depend on `org.springframework.*` (except `org.springframework.modulith.events.*` for `@DomainEvents`), `org.springframework.web.*`, `com.fasterxml.jackson.*`, `jakarta.servlet.*`; (2) `web/` may not depend on `..infra..` or `..domain..` of another module; (3) `..application..` may not depend on `..web..`; (4) cross-module access goes through `<module>` root package only, never into `<module>.domain` / `<module>.infra` / `<module>.web` directly. Co-locates with S-024 cross-tenant leakage CI.
  - **Re-refine S-022:** the speculative refinement of S-022 (`tenant-id-resolver`) plants the first batch of `@TenantId` aggregates. Refine after this ADR lands so those aggregates ship in `domain/` from the first commit, not as a later reshape.
  - **Refinement template:** `/modernize-refine` solution-architect output for any backend story now lists target packages per concern (`domain/`, `application/`, `web/`, `infra/`) rather than a flat file list. Skill-prompt nudge, not a new story.
  - **CONVENTIONS.md entry:** one-paragraph rule "every module ships `domain/`, `application/`, `web/`, `infra/`; ArchUnit + Modulith verify in CI." Lands in the same boyscout PR as the S-048 reshape.
