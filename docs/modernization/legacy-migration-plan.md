...
| User | Users | `t_user` (V2) | port-as-rows | S-170 | `keycloak_sub` minted by S-028 bulk-provision; passwords NEVER copied (C14). Legacy KC-shadow columns (`lockout_*`, `access_failed_count`, `two_factor_enabled`, `phone_number_confirmed`, `email_confirmed`) NOT mapped — KC owns those; importer maps legacy `email_confirmed=true` → KC `emailVerified=true` at provisioning. Destination string synced to `t_user` by S-170 (project-wide `t_` prefix sweep). Also touched by S-052 (original `t_user` rename for the Postgres reserved word). | [columns](legacy-tables/Users/) |
