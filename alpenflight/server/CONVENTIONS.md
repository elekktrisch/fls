# Conventions — `alpenflight/server/`
...
- **Tenant carrier bypass** — production code calling `TenantContextCarrier.set` from outside `platform.tenancy` → caught by `src/test/java/ch/alpenflight/arch/TenantBypassGuardTest.java`. Runs as part of `./gradlew test`.
...
- **Isolation rule (ADR 0021):** tests own their data per-tenant
  ...
  **pre-clean at start, never teardown**. No `@AfterEach`, no global
  ...
  boundaries). Failed-test state stays in pgAdmin (`alpenflight/ops/dev-up-full.sh`)
...
- **Native SQL bypasses `@TenantId`:** `@Query(nativeQuery=true)`, `JdbcTemplate`, raw `DataSource.getConnection()` are NOT filtered by Hibernate. The `ch.alpenflight.platform.tenancy` JDBC aisle is the legitimate exception — `UserPrincipalLookup` (cross-tenant `t_user` reads by `keycloak_sub`) runs inside Hibernate's session-open path where a JPA query would recurse, and `t_user` carries no `@TenantId`. (`LanguageCodeLookup` was JPA-converted at J-26 T-14 — it runs in the JIT filter, not the session-open path, so it reads through the `LanguageRepository` port; ADR 0027.) Any further native SQL in tenant-scoped code is a regression; S-024 will add the repo-wide CI sweep and an explicit `@UnscopedNativeQuery` marker for legitimate exceptions.
...
3. **Sysadmin override + per-tenant SpEL gate** — `@PreAuthorize("hasRole('SYSTEM_ADMINISTRATOR') or ((hasRole('CLUB_ADMINISTRATOR') or hasRole('FLIGHT_OPERATOR')) and @tenant.isOwnClub(#id))")`. The default shape for per-tenant operations on the Clubs aggregate root. `@tenant` is `OwnTenantAuthorization` (`ch.alpenflight.platform.tenancy`); it compares the typed ID against the tenant `ClubTenantIdentifierResolver` resolves for the principal — the same value `@TenantId` filters on. **Do not compare the raw `clubId` claim** (`#id.value().toString() == principal.claims['clubId']`): the claim is a tenant hint that may hold a club key (the realm seeds `clubId=club-1`) or be absent on a federated token, so the string comparison denies a principal access to its own club.
...
- **`@PreAuthorize` SpEL with typed IDs:** pass the typed ID itself to `@tenant.isOwnClub(#id)` (§Authorization patterns) rather than string-comparing against the `clubId` claim — the claim's form is not guaranteed to be the id's.
...
2. **Deliberate tombstone coverage goes in the index NAME, not a comment:** when an index covers tombstones on purpose, the reason is a suffix on the index name — `ix_pda_planning_day` → `ix_pda_planning_day_covers_tombstones_cascade_target`. Names are enforced by `\d` and by every later reader; a `-- covers tombstones: <reason>` comment was neither, and the no-comments rule drops it. Postgres identifiers cap at 63 bytes: abbreviate the table/column half before the reason half. Renaming an already-applied index takes a new migration (`ALTER INDEX … RENAME TO …`), never an edit to the applied one. Three known reasons:
    - **CASCADE join target** — suffix `_covers_tombstones_cascade_target`: an index on a child-side FK whose parent's `ON DELETE CASCADE` runs needs to find soft-deleted children too, so the cascade reaches them. Example: search for `CREATE INDEX ix_pda_planning_day` in `src/main/resources/db/migration/V4__reservations_planning_accounting.sql`.
    - **Tables with no soft-delete** — suffix `_covers_all_rows_no_soft_delete`: snapshot / append-only / CASCADE-only tables don't carry `deleted_on`; indexes on them cover all rows by construction. Example: search for `CREATE INDEX ix_dcti_test` in `V4__reservations_planning_accounting.sql`.
    - **Deferred perf tuning (case-by-case)** — suffix `_covers_tombstones_pending_plan_analysis`: index shape pending production-scale plan analysis. Example: search for `CREATE INDEX ix_arv_location` in `V4__reservations_planning_accounting.sql` (deferred to S-108).
...
Synthetic placeholders only. No realistic Swiss names, emails, phones, licence numbers, or club fixtures. The committed `alpenflight/web/openapi/openapi.json` is a public artifact in the repo's post-squash history; an example that ships there is forever-public.
...
- The committed `alpenflight/web/openapi/openapi.json` is the artifact S-004's codegen reads from.
