...
- **Native SQL bypasses `@TenantId`:** `@Query(nativeQuery=true)`, `JdbcTemplate`, raw `DataSource.getConnection()` are NOT filtered by Hibernate. The `ch.alpenflight.platform.tenancy` JDBC aisle is the legitimate exception — `UserPrincipalLookup` (cross-tenant `t_user` reads by `keycloak_sub`) and `LanguageCodeLookup` (reference-table `language` reads by `code`) run inside Hibernate's session-open path where a JPA query would recurse, and neither table carries `@TenantId`. Any further native SQL in tenant-scoped code is a regression; S-024 will add the repo-wide CI sweep and an explicit `@UnscopedNativeQuery` marker for legitimate exceptions.
