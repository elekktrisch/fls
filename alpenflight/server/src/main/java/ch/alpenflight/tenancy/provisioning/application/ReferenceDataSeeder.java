 ...
 * (S-138). Inserts the TENANT_SCOPED defaults a self-service tenant
 ...
 *       {@code private}, {@code ferry}, with the
 *       glider / tow / motor / observer flags set to legacy defaults.</li>
 * </ul>
 *
 * <p>{@code t_flight_cost_balance_type} is intentionally NOT seeded
 * here: V3 baseline populates it as a system-global catalogue (no
 * {@code club_id} column), shared across tenants per the S-047
 * reference-data pattern.
 *
 * <p>The bundle-wins-on-conflict rule (refinement, S-141 grilled) is
 * enforced by {@code ON CONFLICT (...) DO NOTHING}: if a bundle already
 * inserted a row with the same identity key, the default is skipped.
 * The matching UNIQUE indexes are
 * {@code ux_member_state_club_name} (V15) and
 * {@code ux_flight_type_club_name} (V11).
 *
 ...
 * with the {@code club_id} passed in explicitly. {@code REQUIRES_NEW} is
 * NOT set; the seeder joins the caller's transaction so a rollback
 * after seeding rewinds the inserts cleanly.
 */
@Component
public class ReferenceDataSeeder {
    ...
    private void seedFlightTypes(UUID clubId) {
        // Flag tuples mirror the legacy "small four" defaults the
        // walking-skeleton ports surface to a freshly-provisioned tenant.
        // {forGlider, forTow, forMotor, instructorRequired}.
        ...
        for (DefaultFlightType d : defaults) {
            jdbc.update("""
                    ...
                    """,
                    UUID.randomUUID().toString(), clubId.toString(), d.name(),
