 ...
 * step 1: it would scope the failure paths too.
 */
@Service
public class PublicRegistrationIntake {
    ...
    public PublicRegistrationIntake(PublicClubResolver resolver, PublicRegistrationTxWriter writer) {
        this.resolver = resolver;
        this.writer = writer;
    }

    /** Resolves the slug and records the accepted discovery-flight submission. */
    public PublicClub acceptDiscovery(String clubSlug) {
        return accept(clubSlug, PublicRegistrationKind.DISCOVERY_FLIGHT);
    }

    /** Resolves the slug and records the accepted scenic-flight submission. */
    public PublicClub acceptScenic(String clubSlug) {
        return accept(clubSlug, PublicRegistrationKind.SCENIC_FLIGHT);
    }

    private PublicClub accept(String clubSlug, PublicRegistrationKind kind) {
