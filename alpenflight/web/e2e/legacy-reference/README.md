...
| `reporting`    | J-7     | legacy `flsweb` reporting picker + canned-result + custom-builder (`reporting/picker.png` · `result.png` · `custom.png`) — `e2e/tests/reporting/reporting-parity-J7.spec.ts`. NOT committed yet (the legacy Node-8 `flsweb` + MSSQL stack is unavailable on the Alpine/musl dev box — no browser, no Mono/MSSQL). The parity capture spec is authored against the REAL legacy selectors + staged at the gate; its first live green is the fan-out run that brings up the stack, which commits these PNGs here. Until then CI's `add_pair` degrades to the AlpenFlight side.     | — (deferred — staged via `reporting-parity-J7.spec.ts`; committed once a fanout run captures them)  |
| `accounting`   | J-8     | legacy `flsweb` accounting-rule-filter list + edit form (`accounting/list.png` · `form.png`) — `e2e/tests/accounting/accounting-parity-J8.spec.ts`. NOT committed yet (the legacy Node-8 `flsweb` + MSSQL stack is unavailable on the Alpine/musl dev box — no browser, no Mono/MSSQL). The parity capture spec is authored against the REAL legacy `masterdata/accountingRules/` selectors + staged at the gate; its first live green is the fan-out run that brings up the stack, which commits these PNGs here. Until then CI's `add_shot` degrades to the AlpenFlight side. | — (deferred — staged via `accounting-parity-J8.spec.ts`; committed once a fanout run captures them) |

| `lostpassword` | J-19    | legacy `flsweb` `/lostpassword` form + its success message + the `/confirm` choose-password form (`lostpassword/form.png` · `success.png` · `confirm.png`) — `e2e/tests/auth/lostpassword-parity-J19.spec.ts`. Not committed yet: the Node-8 `flsweb` + Mono `flsserver` + MSSQL stack does not run on this dev box. The fan-out step "Run J-19 T-14 legacy password-recovery capture spec" captures them. **Capture only — no parity claim:** ADR 0007 gives every credential action to Keycloak, so AlpenFlight has no matching screen to pair against.                                                                                       | — (deferred — staged via `lostpassword-parity-J19.spec.ts`; committed once a fanout run captures them) |
