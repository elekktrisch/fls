...
test.describe("rules-engine per-type coverage", () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    ...
    expect(
      tokenRes.ok(),
      `token: ${tokenRes.status()} ${await tokenRes.text()}`,
    ).toBeTruthy();
    ...
    for (const id of FIXTURE_RULE_IDS) {
      ...
      expect(
        putRes.ok(),
        `deactivate fixture rule ${id}: ${putRes.status()}`,
      ).toBeTruthy();
    }
  });
  ...
  async function createRule(payload: Record<string, unknown>): Promise<string> {
    ...
    expect(
      res.ok(),
      `insert rule: ${res.status()} ${await res.text()}`,
    ).toBeTruthy();
  ...
  async function deleteRule(id: string): Promise<void> {
    await api.delete(`${API_BASE}/api/v1/accountingrulefilters/${id}`, {
      headers: auth,
    });
  }
  ...
  async function previewDelivery(): Promise<DeliveryCreationResult> {
    ...
    expect(
      res.ok(),
      `preview: ${res.status()} ${await res.text()}`,
    ).toBeTruthy();
  ...
  test("Recipient sets RecipientDetails on the delivery", async () => {
    ...
    try {
      const r = await previewDelivery();
      expect(r.MatchedAccountingRuleFilterIds).toContain(id);
      expect(r.CreatedDeliveryDetails.RecipientDetails.RecipientName).toBe(
        recipientName,
      );
    } finally {
      await deleteRule(id);
  ...
  test("FlightTime emits one item with quantity = flight duration", async () => {
    ...
    try {
      // The just-inserted rule can lag the delivery-preview engine's first read
      // under load, so re-fetch the preview until the rule matches, then assert
      // the emitted item on that settled result (the whole preview is derived
      // from one rules-engine pass — re-reading yields a self-consistent `r`).
      let r!: DeliveryCreationResult;
      await expect(async () => {
        r = await previewDelivery();
        expect(r.MatchedAccountingRuleFilterIds).toContain(id);
      }).toPass({ timeout: 15_000 });
      const items = r.CreatedDeliveryDetails.DeliveryItems.filter(
  ...
  test("EngineTime — skipped: glider seed flight has no engine times", async () => {
    ...
    test.skip(
      true,
      "glider seed flight has no engine times to drive the EngineTime loop",
    );
  });
